...
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_width;
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_height;
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_left;
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_top;
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_right;
@property (nonatomic, strong) SDAutoLayoutModelItem *sd_bottom;
...
{
    if (!_leftSpaceToView) {
        _leftSpaceToView = [self marginToViewblockWithKey:@"sd_left"];
    }
...
- (MarginToView)rightSpaceToView
{
    if (!_rightSpaceToView) {
        _rightSpaceToView = [self marginToViewblockWithKey:@"sd_right"];
    }
...
{
    if (!_topSpaceToView) {
        _topSpaceToView = [self marginToViewblockWithKey:@"sd_top"];
    }
...
- (MarginToView)bottomSpaceToView
{
    if (!_bottomSpaceToView) {
        _bottomSpaceToView = [self marginToViewblockWithKey:@"sd_bottom"];
    }
...
- (void)sd_layoutSubviewsHandle{
    ...
    if (self.sd_equalWidthSubviews.count) {
        ...
        [self.sd_equalWidthSubviews enumerateObjectsUsingBlock:^(UIView *view, NSUInteger idx, BOOL *stop) {
            ...
            CGFloat left = model.sd_left ? [model.sd_left.value floatValue] : model.needsAutoResizeView.left_sd;
            totalMargin += (left + [model.sd_right.value floatValue]);
        }];
    ...
    if (self.tag == kSDModelCellTag && [self isKindOfClass:cellContVClass()]) {
        ...
        if (![self isKindOfClass:[UIScrollView class]] && self.sd_rightViewsArray.count && (model.sd_right || model.equalRight || model.centerX || model.equalCenterX)) {
            self.fixedWidth = @(self.sd_width);
            if (model.sd_right || model.equalRight) {
        ...
        if (![self isKindOfClass:[UIScrollView class]] && self.sd_bottomViewsArray.count && (model.sd_bottom || model.equalBottom || model.centerY || model.equalCenterY)) {
            self.fixedHeight = @(self.sd_height);
            if (model.sd_bottom || model.equalBottom) {
...
{
    if (model.sd_width) {
        view.width_sd = [model.sd_width.value floatValue];
        view.fixedWidth = @(view.width_sd);
...
{
    if (model.sd_height) {
        view.height_sd = [model.sd_height.value floatValue];
        view.fixedHeight = @(view.height_sd);
...
{
    if (model.sd_left) {
        if (view.superview == model.sd_left.refView) {
            if (!view.fixedWidth) { // view.autoLeft && view.autoRight
                view.width_sd = view.right_sd - [model.sd_left.value floatValue];
            }
            view.left_sd = [model.sd_left.value floatValue];
        } else {
            if (model.sd_left.refViewsArray.count) {
                CGFloat lastRefRight = 0;
                for (UIView *ref in model.sd_left.refViewsArray) {
                    if ([ref isKindOfClass:[UIView class]] && ref.right_sd > lastRefRight) {
                        model.sd_left.refView = ref;
                        lastRefRight = ref.right_sd;
                    }
                }
            }
            if (!view.fixedWidth) { // view.autoLeft && view.autoRight
                view.width_sd = view.right_sd - model.sd_left.refView.right_sd - [model.sd_left.value floatValue];
            }
            view.left_sd = model.sd_left.refView.right_sd + [model.sd_left.value floatValue];
        }
...
{
    if (model.sd_right) {
        if (view.superview == model.sd_right.refView) {
            if (!view.fixedWidth) { // view.autoLeft && view.autoRight
                view.width_sd = model.sd_right.refView.width_sd - view.left_sd - [model.sd_right.value floatValue];
            }
            view.right_sd = model.sd_right.refView.width_sd - [model.sd_right.value floatValue];
        } else {
            if (!view.fixedWidth) { // view.autoLeft && view.autoRight
                view.width_sd =  model.sd_right.refView.left_sd - view.left_sd - [model.sd_right.value floatValue];
            }
            view.right_sd = model.sd_right.refView.left_sd - [model.sd_right.value floatValue];
        }
...
{
    if (model.sd_top) {
        if (view.superview == model.sd_top.refView) {
            if (!view.fixedHeight) { // view.autoTop && view.autoBottom && view.bottom
                view.height_sd = view.bottom_sd - [model.sd_top.value floatValue];
            }
            view.top_sd = [model.sd_top.value floatValue];
        } else {
            if (model.sd_top.refViewsArray.count) {
                CGFloat lastRefBottom = 0;
                for (UIView *ref in model.sd_top.refViewsArray) {
                    if ([ref isKindOfClass:[UIView class]] && ref.bottom_sd > lastRefBottom) {
                        model.sd_top.refView = ref;
                        lastRefBottom = ref.bottom_sd;
                    }
                }
            }
            if (!view.fixedHeight) { // view.autoTop && view.autoBottom && view.bottom
                view.height_sd = view.bottom_sd - model.sd_top.refView.bottom_sd - [model.sd_top.value floatValue];
            }
            view.top_sd = model.sd_top.refView.bottom_sd + [model.sd_top.value floatValue];
        }
...
{
    if (model.sd_bottom) {
        if (view.superview == model.sd_bottom.refView) {
            if (!view.fixedHeight) {
                view.height_sd = view.superview.height_sd - view.top_sd - [model.sd_bottom.value floatValue];
            }
            view.bottom_sd = model.sd_bottom.refView.height_sd - [model.sd_bottom.value floatValue];
        } else {
            if (!view.fixedHeight) {
                view.height_sd = model.sd_bottom.refView.top_sd - view.top_sd - [model.sd_bottom.value floatValue];
            }
            view.bottom_sd = model.sd_bottom.refView.top_sd - [model.sd_bottom.value floatValue];
        }
